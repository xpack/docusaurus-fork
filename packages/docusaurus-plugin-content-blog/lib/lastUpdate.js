...
async function getFileLastUpdate(filePath) {
    ...
    try {
        const result = (0, docusaurus_utils_1.getFileCommitDate)(filePath, {
            age: 'newest',
            includeAuthor: true,
        });
        ...
        if (err instanceof docusaurus_utils_1.GitNotFoundError) {
            if (!showedGitRequirementError) {
                logger_1.default.warn('Sorry, the docs plugin last update options require Git.');
                showedGitRequirementError = true;
            }
        }
        else if (err instanceof docusaurus_utils_1.FileNotTrackedError) {
