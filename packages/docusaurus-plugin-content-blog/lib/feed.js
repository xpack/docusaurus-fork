...
async function generateBlogFeed({ blogPosts, options, siteConfig, outDir, locale, }) {
    ...
    const blogBaseUrl = (0, docusaurus_utils_1.normalizeUrl)([siteUrl, baseUrl, routeBasePath]);
    ...
    const feed = new feed_1.Feed({
        ...
        favicon: favicon ? (0, docusaurus_utils_1.normalizeUrl)([siteUrl, baseUrl, favicon]) : undefined,
...
async function defaultCreateFeedItems({ blogPosts, siteConfig, outDir, }) {
    ...
    return Promise.all(blogPosts.map(async (post) => {
        ...
        const content = await (0, docusaurus_utils_1.readOutputHTMLFile)(permalink.replace(siteConfig.baseUrl, ''), outDir, siteConfig.trailingSlash);
        const $ = (0, cheerio_1.load)(content);
        const blogPostAbsoluteUrl = (0, docusaurus_utils_1.normalizeUrl)([siteUrl, permalink]);
...
async function createBlogFeedFiles({ blogPostsNewest, options, siteConfig, outDir, locale, }) {
